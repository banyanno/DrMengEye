Public Class DashboardDoctorConsult
    ...
    Private Sub ViewPatientVANewExam(ByVal WaitingID As Double)
        ...
        For Each row As DataRow In tblVA.Rows
            ...
            TxtLLE.Text = row("LLE")
            TxtIOPLE.Text = row("IOPLE")
            TxtIOPRE.Text = row("IOPRE")
    ...
    Private Sub ViewPatientVAHistory(ByVal WaitingID As Double)
        ...
        For Each row As DataRow In tblVA.Rows
            ...
            Try
                ...
                TxtLaboratory.Text = row("LABORATORY")
                TxtLaser.Text = row("LASER")
                TxtInvestigate.Text = row("INVESTIGATE")
            Catch ex As Exception
    ...
    Private Sub ViewConsultDetail()
        ...
        Try
            CboDiagnosis.Text = CStr(IIf(TypeOf (GridHistoy.GetRow.Cells("DIAGNOSIS").Value) Is DBNull, "", GridHistoy.GetRow.Cells("DIAGNOSIS").Value))
            CboDiagnosisLeft.Text = CStr(IIf(TypeOf (GridHistoy.GetRow.Cells("DIAGNOSIS_LEFT_EYE").Value) Is DBNull, "", GridHistoy.GetRow.Cells("DIAGNOSIS_LEFT_EYE").Value))
            TxtDiagnosisCodeLeft.Text = CStr(IIf(TypeOf (GridHistoy.GetRow.Cells("DIAGNOSIS_CODE_LEFT").Value) Is DBNull, "", GridHistoy.GetRow.Cells("DIAGNOSIS_CODE_LEFT").Value))
            TxtDiagnosisCodeRight.Text = CStr(IIf(TypeOf (GridHistoy.GetRow.Cells("DIAGNOSIS_CODE").Value) Is DBNull, "", GridHistoy.GetRow.Cells("DIAGNOSIS_CODE").Value))
            Dim IS_Diagnosis As String = CStr(IIf(TypeOf (GridHistoy.GetRow.Cells("IS_ON_EYE").Value) Is DBNull, "", GridHistoy.GetRow.Cells("IS_ON_EYE").Value))
    ...
    Sub NewPatientExam()
        ...
        NConsult.TxtAgePatient.Text = GridPatientWaiting.GetRow.Cells("CalAge").Value
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEyeExam.Click
        If GridPatientWaiting.SelectedItems.Count = 0 Then Exit Sub
